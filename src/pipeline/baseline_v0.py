...
from src.preprocessing.customer_profile import preprocess_customer_profiles
...
def run_baseline_v0(config: Mapping[str, Any], *, resolved_config_path: str | Path) -> dict[str, Any]:
    ...
    scoring_panel = _materialize_scoring_panel(split, artifacts / "scoring_panel.parquet", runtime)
    model = run_popularity_baseline(scoring_panel, artifacts, validation_date=str(config["split"]["validation_date"]), top_k=int(config["model"]["top_k"]), memory_limit=runtime.get("memory_limit"), temp_directory=_temp_path(runtime))
    ...
    manifest = build_run_manifest(run_id=run_id, config=config, inputs={"raw_train": raw_path, "source_train": interim["interim"], "split_train": split["train"]}, outputs={"profile_train": profiles["train"], "split_validation_input": split["validation_input"], "split_validation_target": split["validation_target"], "metrics": model["metrics_path"], "ranking": artifacts / "popularity_ranking.parquet", "predictions": artifacts / "validation_predictions.parquet", "target": artifacts / "validation_target.parquet", "resolved_config": resolved_copy})
